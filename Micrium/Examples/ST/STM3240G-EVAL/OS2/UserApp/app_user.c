...
static  void  AppTaskEvent0 (void  *p_arg)
{
    ...
    while (DEF_TRUE) {
        ...
        APP_TRACE_INFO(("T0 MtxPend\n"));
        UartSendChar('X');

        OSTimeDlyHMSM(0u, 0u, 0u, 100u);

        os_err = OSMutexPost(AppTaskObjMutex);
        APP_TRACE_INFO(("T0 MtxPost\n"));
#endif

#if (OS_Q_EN > 0u)
        os_err = OSQPost(        AppTaskObjQ,
                         (void *)1u);
        APP_TRACE_INFO(("T0 QPost\n"));
#endif
        ...
        OSFlagPost( AppTaskObjFlag,
                    ...
                    OS_FLAG_SET,
                   &os_err);
        APP_TRACE_INFO(("T0 FPost\n"));
#endif
        APP_TRACE_INFO(("T0 Dly\n"));
        OSTimeDlyHMSM(0u, 0u, 0u, 10u);

        pcfg = (SERIAL_DEV_CFG *)&SerialDevCfg_STM32_USART2;

        x = pcfg->BSP_API->ClkFreqGet();

        //getRandom = rand(256);

        APP_TRACE_INFO(("Event test task 0 running ....\n"));
       // APP_TRACE_INFO(("UART2 brr %x....\n",*(unsigned int*)(0x40004408)));
        //APP_TRACE_INFO(("Get UART2 Clk %d....\n",x));
       // APP_TRACE_INFO(("TEST PRINTF %x\n",x));
    }
}
*/
...
static  void  AppTaskEvent1 (void  *p_arg)
{
    ...
    while (DEF_TRUE) {
        ...
        OSMutexPend( AppTaskObjMutex,
                     0,
                    &os_err);
        APP_TRACE_INFO(("T1 MtxPend\n"));

        os_err = OSMutexPost(AppTaskObjMutex);
        APP_TRACE_INFO(("T1 MtxPost\n"));
#endif
        ...
        OSFlagPend( AppTaskObjFlag,
                    ...
                    0,
                   &os_err);
        APP_TRACE_INFO(("T0 FPend\n"));
#endif
        APP_TRACE_INFO(("T1 Dly\n"));
        OSTimeDlyHMSM(0u, 0u, 0u, 10u);
...
void  AppTaskEq0Fp (void  *p_arg)
{
	while (DEF_TRUE)
	{
		...
		UserPrint("\x1B[41m");
		UserPrintXY(0,AppProbe_CtrVal,5,"Task1");
	}
}
...
void  AppTaskEq1Fp (void  *p_arg)
{
	while (DEF_TRUE)
	{
		...
		UserPrint("\x1B[42m");
		UserPrintXY(0,AppProbe_CtrVal,9,"Task2");
	}
}
//void  AppTaskEq0Fp (void  *p_arg)
//{
