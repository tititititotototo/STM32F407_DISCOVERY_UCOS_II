...
static void LedTask(void *p_arg)
{
	...
	while(DEF_TRUE)
	{
		...
		UserPrint("Led Task Use ShareData Prio %d\n",LED_TASK_PRIO);
		sprintf((char*)Sharedata,"Led Task write");
		OSTimeDlyHMSM(0u, 0u, 0u,500u);

		UserPrint("Led Task :: %s\n",Sharedata);
...
static void FloatTask(void *p_arg)
{
	...
	{
		OSSemPend(ShareDataSem,0,&err);
		UserPrint("Float Task Use ShareData Prio %d\n",FLOAT_TASK_PRIO);
		sprintf((char*)Sharedata,"Float Task write");
		OSTimeDlyHMSM(0u, 0u, 0u,500u);

		UserPrint("Float Task :: %s\n\n",Sharedata);
