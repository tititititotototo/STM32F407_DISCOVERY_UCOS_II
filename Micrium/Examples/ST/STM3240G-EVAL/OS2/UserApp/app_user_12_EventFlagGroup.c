	...
	while(DEF_TRUE)
	{
		...
		else if ((UartReady == SET) && (rxStartFlag == 1))
		{
			...
			switch(aRxBuffer[0])
			{
				...
				case '4':
					OSFlagPost(EventFlag,DEF_BIT_00,OS_FLAG_SET,&err);
					break;

				case '5':
					OSFlagPost(EventFlag,DEF_BIT_01,OS_FLAG_SET,&err);
					break;
...
static void LedTask(void *p_arg)
{
	...
	while(DEF_TRUE)
	{
		OSFlagPend(EventFlag,DEF_BIT(0) | DEF_BIT(1),OS_FLAG_WAIT_SET_ALL + OS_FLAG_CONSUME,0,&err);
		UserPrint("Led Task Get The Flag !! (Prio : %d)\n",LED_TASK_PRIO);
...
static void FloatTask(void *p_arg)
{
	...
	while(DEF_TRUE)
	{
		OSFlagPend(EventFlag,DEF_BIT(0) | DEF_BIT(1),OS_FLAG_WAIT_SET_ANY + OS_FLAG_CONSUME,0,&err);
		UserPrint("Float Task Get The Flag !! (Prio : %d)\n",FLOAT_TASK_PRIO);
